...
RSpec.describe User, type: :model do
  ...
  it 'is invalid with a non-unique email' do
    ...
    user = User.new(email: 'test@example.com', username: 'anotheruser', password: 'password123')
    expect(user).not_to be_valid
  end
  ...
  it 'is invalid with a non-unique username' do
    User.create(email: 'test@example.com', username: 'testuser', password: 'password123')
    user = User.new(email: 'another@example.com', username: 'testuser', password: 'password123')
    expect(user).not_to be_valid
  end
